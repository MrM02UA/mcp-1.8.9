...
public class EnchantmentProtection extends Enchantment
{
    ...
    public int calcModifierDamage(int level, DamageSource source)
    {
        ...
        {
            float lvt_3_1_ = (float)(6 + level * level) / 3.0F;
            return this.protectionType == 0 ? MathHelper.floor_float(lvt_3_1_ * 0.75F) : (this.protectionType == 1 && source.isFireDamage() ? MathHelper.floor_float(lvt_3_1_ * 1.25F) : (this.protectionType == 2 && source == DamageSource.fall ? MathHelper.floor_float(lvt_3_1_ * 2.5F) : (this.protectionType == 3 && source.isExplosion() ? MathHelper.floor_float(lvt_3_1_ * 1.5F) : (this.protectionType == 4 && source.isProjectile() ? MathHelper.floor_float(lvt_3_1_ * 1.5F) : 0))));
        }
    }
    ...
    public boolean canApplyTogether(Enchantment ench)
    {
        ...
        {
            EnchantmentProtection lvt_2_1_ = (EnchantmentProtection)ench;
            return lvt_2_1_.protectionType == this.protectionType ? false : this.protectionType == 2 || lvt_2_1_.protectionType == 2;
        }
        else
        {
    ...
    {
        int lvt_2_1_ = EnchantmentHelper.getMaxEnchantmentLevel(Enchantment.fireProtection.effectId, p_92093_0_.getInventory());

        if (lvt_2_1_ > 0)
        {
            p_92093_1_ -= MathHelper.floor_float((float)p_92093_1_ * (float)lvt_2_1_ * 0.15F);
        }
    ...
    {
        int lvt_3_1_ = EnchantmentHelper.getMaxEnchantmentLevel(Enchantment.blastProtection.effectId, p_92092_0_.getInventory());

        if (lvt_3_1_ > 0)
        {
            p_92092_1_ -= (double)MathHelper.floor_double(p_92092_1_ * (double)((float)lvt_3_1_ * 0.15F));
        }
