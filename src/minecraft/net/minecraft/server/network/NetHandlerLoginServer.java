...
public class NetHandlerLoginServer implements INetHandlerLoginServer, ITickable
{
    ...
    public void update()
    {
        ...
        {
            EntityPlayerMP lvt_1_1_ = this.server.getConfigurationManager().getPlayerByUUID(this.loginGameProfile.getId());
    ...
    public void closeConnection(String reason)
    {
        try
        {
            ...
            ChatComponentText lvt_2_1_ = new ChatComponentText(reason);
            this.networkManager.sendPacket(new S00PacketDisconnect(lvt_2_1_));
            this.networkManager.closeChannel(lvt_2_1_);
        }
    ...
    public void tryAcceptPlayer()
    {
        ...
        String lvt_1_1_ = this.server.getConfigurationManager().allowUserToConnect(this.networkManager.getRemoteAddress(), this.loginGameProfile);

        if (lvt_1_1_ != null)
        {
            this.closeConnection(lvt_1_1_);
        }
        else
        {
            ...
            EntityPlayerMP lvt_2_1_ = this.server.getConfigurationManager().getPlayerByUUID(this.loginGameProfile.getId());
    ...
    public void processEncryptionResponse(C01PacketEncryptionResponse packetIn)
    {
        ...
        PrivateKey lvt_2_1_ = this.server.getKeyPair().getPrivate();

        if (!Arrays.equals(this.verifyToken, packetIn.getVerifyToken(lvt_2_1_)))
        {
            throw new IllegalStateException("Invalid nonce!");
        }
        else
        {
            this.secretKey = packetIn.getSecretKey(lvt_2_1_);
            ...
            (new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet())
            {
                public void run()
                {
                    GameProfile lvt_1_1_ = NetHandlerLoginServer.this.loginGameProfile;

                    try
                    {
                        String lvt_2_1_ = (new BigInteger(CryptManager.getServerIdHash(NetHandlerLoginServer.this.serverId, NetHandlerLoginServer.this.server.getKeyPair().getPublic(), NetHandlerLoginServer.this.secretKey))).toString(16);
                        NetHandlerLoginServer.this.loginGameProfile = NetHandlerLoginServer.this.server.getMinecraftSessionService().hasJoinedServer(new GameProfile((UUID)null, lvt_1_1_.getName()), lvt_2_1_);
                        ...
                        else if (NetHandlerLoginServer.this.server.isSinglePlayer())
                        {
                            ...
                            NetHandlerLoginServer.this.loginGameProfile = NetHandlerLoginServer.this.getOfflineProfile(lvt_1_1_);
                            NetHandlerLoginServer.this.currentLoginState = NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
                        }
                        else
                        {
    ...
    {
        UUID lvt_2_1_ = UUID.nameUUIDFromBytes(("OfflinePlayer:" + original.getName()).getBytes(Charsets.UTF_8));
        return new GameProfile(lvt_2_1_, original.getName());
    }
