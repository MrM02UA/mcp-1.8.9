...
public abstract class BanEntry<T> extends UserListEntry<T>
{
    ...
    protected BanEntry(T valueIn, JsonObject json)
    {
        ...
        try
        {
            lvt_3_1_ = json.has("created") ? dateFormat.parse(json.get("created").getAsString()) : new Date();
        }
        ...
        this.banStartDate = lvt_3_1_;
        this.bannedBy = json.has("source") ? json.get("source").getAsString() : "(Unknown)";
        Date lvt_4_2_;

        try
        {
            lvt_4_2_ = json.has("expires") ? dateFormat.parse(json.get("expires").getAsString()) : null;
        }
