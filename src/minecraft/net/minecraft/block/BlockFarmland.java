...
public class BlockFarmland extends Block
{
    ...
    {
        int lvt_5_1_ = ((Integer)state.getValue(MOISTURE)).intValue();

        if (!this.hasWater(worldIn, pos) && !worldIn.isRainingAt(pos.up()))
        {
            if (lvt_5_1_ > 0)
            {
                worldIn.setBlockState(pos, state.withProperty(MOISTURE, Integer.valueOf(lvt_5_1_ - 1)), 2);
            }
    ...
    {
        Block lvt_3_1_ = worldIn.getBlockState(pos.up()).getBlock();
        return lvt_3_1_ instanceof BlockCrops || lvt_3_1_ instanceof BlockStem;
    }

    private boolean hasWater(World worldIn, BlockPos pos)
    {
        for (BlockPos.MutableBlockPos lvt_4_1_ : BlockPos.getAllInBoxMutable(pos.add(-4, 0, -4), pos.add(4, 1, 4)))
        {
            if (worldIn.getBlockState(lvt_4_1_).getBlock().getMaterial() == Material.water)
            {
                return true;
            }
        }
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side)
    {
        switch (side)
        {
            ...
            case EAST:
                ...
                return !lvt_4_1_.isOpaqueCube() && lvt_4_1_ != Blocks.farmland;
