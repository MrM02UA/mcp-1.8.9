...
public class BlockPistonExtension extends Block
{
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
    {
        if (player.capabilities.isCreativeMode)
        {
            ...
            if (lvt_5_1_ != null)
            {
                BlockPos lvt_6_1_ = pos.offset(lvt_5_1_.getOpposite());
                Block lvt_7_1_ = worldIn.getBlockState(lvt_6_1_).getBlock();

                if (lvt_7_1_ == Blocks.piston || lvt_7_1_ == Blocks.sticky_piston)
                {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        pos = pos.offset(lvt_4_1_);
        IBlockState lvt_5_1_ = worldIn.getBlockState(pos);

        if ((lvt_5_1_.getBlock() == Blocks.piston || lvt_5_1_.getBlock() == Blocks.sticky_piston) && ((Boolean)lvt_5_1_.getValue(BlockPistonBase.EXTENDED)).booleanValue())
        {
            lvt_5_1_.getBlock().dropBlockAsItem(worldIn, pos, lvt_5_1_, 0);
    ...
    {
        float lvt_2_1_ = 0.25F;
        EnumFacing lvt_3_1_ = (EnumFacing)state.getValue(FACING);
    ...
    public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock)
    {
        ...
        if (lvt_7_1_.getBlock() != Blocks.piston && lvt_7_1_.getBlock() != Blocks.sticky_piston)
        {
            worldIn.setBlockToAir(pos);
        }
        else
        {
            lvt_7_1_.getBlock().onNeighborBlockChange(worldIn, lvt_6_1_, lvt_7_1_, neighborBlock);
        }
    }
