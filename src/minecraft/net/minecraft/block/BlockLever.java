...
public class BlockLever extends Block
{
    ...
    public boolean canPlaceBlockAt(World worldIn, BlockPos pos)
    {
        ...
        {
            if (func_181090_a(worldIn, pos, lvt_6_1_))
            {
                return true;
            }
        }
    ...
    {
        IBlockState lvt_9_1_ = this.getDefaultState().withProperty(POWERED, Boolean.valueOf(false));

        if (func_181090_a(worldIn, pos, facing.getOpposite()))
        {
            return lvt_9_1_.withProperty(FACING, BlockLever.EnumOrientation.forFacings(facing, placer.getHorizontalFacing()));
        }
        else
        {
            for (EnumFacing lvt_11_1_ : EnumFacing.Plane.HORIZONTAL)
            {
                if (lvt_11_1_ != facing && func_181090_a(worldIn, pos, lvt_11_1_.getOpposite()))
                {
                    return lvt_9_1_.withProperty(FACING, BlockLever.EnumOrientation.forFacings(lvt_11_1_, placer.getHorizontalFacing()));
                }
            }

            if (World.doesBlockHaveSolidTopSurface(worldIn, pos.down()))
            {
                return lvt_9_1_.withProperty(FACING, BlockLever.EnumOrientation.forFacings(EnumFacing.UP, placer.getHorizontalFacing()));
            }
            else
            {
                return lvt_9_1_;
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        switch ((BlockLever.EnumOrientation)worldIn.getBlockState(pos).getValue(FACING))
        {
            ...
            case UP_X:
                lvt_3_1_ = 0.25F;
                this.setBlockBounds(0.5F - lvt_3_1_, 0.0F, 0.5F - lvt_3_1_, 0.5F + lvt_3_1_, 0.6F, 0.5F + lvt_3_1_);
                break;

            case DOWN_X:
            case DOWN_Z:
                lvt_3_1_ = 0.25F;
                this.setBlockBounds(0.5F - lvt_3_1_, 0.4F, 0.5F - lvt_3_1_, 0.5F + lvt_3_1_, 1.0F, 0.5F + lvt_3_1_);
        }
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            EnumFacing lvt_9_1_ = ((BlockLever.EnumOrientation)state.getValue(FACING)).getFacing();
            worldIn.notifyNeighborsOfStateChange(pos.offset(lvt_9_1_.getOpposite()), this);
            return true;
        }
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        {
            worldIn.notifyNeighborsOfStateChange(pos, this);
            EnumFacing lvt_4_1_ = ((BlockLever.EnumOrientation)state.getValue(FACING)).getFacing();
            worldIn.notifyNeighborsOfStateChange(pos.offset(lvt_4_1_.getOpposite()), this);
        }
    ...
    {
        int lvt_2_1_ = 0;
        lvt_2_1_ = lvt_2_1_ | ((BlockLever.EnumOrientation)state.getValue(FACING)).getMetadata();
    ...
    public static enum EnumOrientation implements IStringSerializable
    {
        ...
        static {
            for (BlockLever.EnumOrientation lvt_3_1_ : values())
            {
