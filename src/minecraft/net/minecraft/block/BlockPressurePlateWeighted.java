...
public class BlockPressurePlateWeighted extends BlockBasePressurePlate
{
    ...
    {
        int lvt_3_1_ = Math.min(worldIn.getEntitiesWithinAABB(Entity.class, this.getSensitiveAABB(pos)).size(), this.field_150068_a);

        if (lvt_3_1_ > 0)
        {
            float lvt_4_1_ = (float)Math.min(this.field_150068_a, lvt_3_1_) / (float)this.field_150068_a;
            return MathHelper.ceiling_float_int(lvt_4_1_ * 15.0F);
        }
        else
        {
            return 0;
