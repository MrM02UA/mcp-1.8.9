...
public abstract class CommandBase implements ICommand
{
    ...
    {
        int lvt_3_1_ = parseInt(input);

        if (lvt_3_1_ < min)
        {
            throw new NumberInvalidException("commands.generic.num.tooSmall", new Object[] {Integer.valueOf(lvt_3_1_), Integer.valueOf(min)});
        }
        else if (lvt_3_1_ > max)
        {
            throw new NumberInvalidException("commands.generic.num.tooBig", new Object[] {Integer.valueOf(lvt_3_1_), Integer.valueOf(max)});
        }
        else
        {
            return lvt_3_1_;
    ...
    {
        long lvt_5_1_ = parseLong(input);

        if (lvt_5_1_ < min)
        {
            throw new NumberInvalidException("commands.generic.num.tooSmall", new Object[] {Long.valueOf(lvt_5_1_), Long.valueOf(min)});
        }
        else if (lvt_5_1_ > max)
        {
            throw new NumberInvalidException("commands.generic.num.tooBig", new Object[] {Long.valueOf(lvt_5_1_), Long.valueOf(max)});
        }
        else
        {
            return lvt_5_1_;
    ...
    public static BlockPos parseBlockPos(ICommandSender sender, String[] args, int startIndex, boolean centerBlock) throws NumberInvalidException
    {
        ...
        return new BlockPos(parseDouble((double)lvt_4_1_.getX(), args[startIndex], -30000000, 30000000, centerBlock), parseDouble((double)lvt_4_1_.getY(), args[startIndex + 1], 0, 256, false), parseDouble((double)lvt_4_1_.getZ(), args[startIndex + 2], -30000000, 30000000, centerBlock));
    }
    ...
    public static double parseDouble(String input) throws NumberInvalidException
    {
        try
        {
            double lvt_1_1_ = Double.parseDouble(input);
    ...
    {
        double lvt_5_1_ = parseDouble(input);

        if (lvt_5_1_ < min)
        {
            throw new NumberInvalidException("commands.generic.double.tooSmall", new Object[] {Double.valueOf(lvt_5_1_), Double.valueOf(min)});
        }
        else if (lvt_5_1_ > max)
        {
            throw new NumberInvalidException("commands.generic.double.tooBig", new Object[] {Double.valueOf(lvt_5_1_), Double.valueOf(max)});
        }
        else
        {
            return lvt_5_1_;
    ...
    {
        EntityPlayerMP lvt_2_1_ = PlayerSelector.matchOnePlayer(sender, username);

        if (lvt_2_1_ == null)
        {
            try
            {
                lvt_2_1_ = MinecraftServer.getServer().getConfigurationManager().getPlayerByUUID(UUID.fromString(username));
            }
        ...
        if (lvt_2_1_ == null)
        {
            lvt_2_1_ = MinecraftServer.getServer().getConfigurationManager().getPlayerByUsername(username);
        }
    ...
    {
        Entity lvt_3_1_ = PlayerSelector.matchOneEntity(commandSender, p_175759_1_, p_175759_2_);
        MinecraftServer lvt_4_1_ = MinecraftServer.getServer();

        if (lvt_3_1_ == null)
        {
            lvt_3_1_ = lvt_4_1_.getConfigurationManager().getPlayerByUsername(p_175759_1_);
        }
        ...
        if (lvt_3_1_ == null)
        {
            try
            {
                ...
                lvt_3_1_ = lvt_4_1_.getEntityFromUuid(lvt_5_1_);

                if (lvt_3_1_ == null)
                {
                    lvt_3_1_ = lvt_4_1_.getConfigurationManager().getPlayerByUUID(lvt_5_1_);
                }
            }
    ...
    public static IChatComponent getChatComponentFromNthArg(ICommandSender sender, String[] args, int index, boolean p_147176_3_) throws PlayerNotFoundException
    {
        ...
        for (int lvt_5_1_ = index; lvt_5_1_ < args.length; ++lvt_5_1_)
        {
            if (lvt_5_1_ > index)
            {
                lvt_4_1_.appendText(" ");
            ...
            IChatComponent lvt_6_1_ = new ChatComponentText(args[lvt_5_1_]);

            if (p_147176_3_)
            {
                IChatComponent lvt_7_1_ = PlayerSelector.matchEntitiesToChatComponent(sender, args[lvt_5_1_]);

                if (lvt_7_1_ == null)
                {
                    if (PlayerSelector.hasArguments(args[lvt_5_1_]))
                    {
                        throw new PlayerNotFoundException();
                    }
                }
            ...
            lvt_4_1_.appendSibling(lvt_6_1_);
        }
    ...
    public static String buildString(String[] args, int startPos)
    {
        ...
        for (int lvt_3_1_ = startPos; lvt_3_1_ < args.length; ++lvt_3_1_)
        {
            if (lvt_3_1_ > startPos)
            {
                lvt_2_1_.append(" ");
            }
    ...
    public static CommandBase.CoordinateArg parseCoordinate(double p_175767_0_, String p_175767_2_, int min, int max, boolean centerBlock) throws NumberInvalidException
    {
        ...
        else
        {
            ...
            if (!lvt_6_1_ || p_175767_2_.length() > 1)
            {
                ...
                lvt_7_1_ += parseDouble(p_175767_2_);

                if (!lvt_9_1_ && !lvt_6_1_ && centerBlock)
                {
                    lvt_7_1_ += 0.5D;
                }
            }
            ...
            if (min != 0 || max != 0)
            {
                if (lvt_7_1_ < (double)min)
                {
                    throw new NumberInvalidException("commands.generic.double.tooSmall", new Object[] {Double.valueOf(lvt_7_1_), Integer.valueOf(min)});
                }

                if (lvt_7_1_ > (double)max)
                {
                    throw new NumberInvalidException("commands.generic.double.tooBig", new Object[] {Double.valueOf(lvt_7_1_), Integer.valueOf(max)});
                }
            }
    ...
    {
        boolean lvt_6_1_ = input.startsWith("~");

        if (lvt_6_1_ && Double.isNaN(base))
        {
            throw new NumberInvalidException("commands.generic.num.invalid", new Object[] {Double.valueOf(base)});
        }
        else
        {
            double lvt_7_1_ = lvt_6_1_ ? base : 0.0D;

            if (!lvt_6_1_ || input.length() > 1)
            {
                boolean lvt_9_1_ = input.contains(".");
    ...
    {
        ResourceLocation lvt_2_1_ = new ResourceLocation(id);
        Item lvt_3_1_ = (Item)Item.itemRegistry.getObject(lvt_2_1_);
    ...
    public static Block getBlockByText(ICommandSender sender, String id) throws NumberInvalidException
    {
        ...
        if (!Block.blockRegistry.containsKey(lvt_2_1_))
        {
            throw new NumberInvalidException("commands.give.block.notFound", new Object[] {lvt_2_1_});
        }
        else
        {
            Block lvt_3_1_ = (Block)Block.blockRegistry.getObject(lvt_2_1_);
    ...
    public static String joinNiceString(Object[] elements)
    {
        ...
        for (int lvt_2_1_ = 0; lvt_2_1_ < elements.length; ++lvt_2_1_)
        {
            String lvt_3_1_ = elements[lvt_2_1_].toString();

            if (lvt_2_1_ > 0)
            {
                if (lvt_2_1_ == elements.length - 1)
                {
                    lvt_1_1_.append(" and ");
                }
                else
    ...
    public static IChatComponent join(List<IChatComponent> components)
    {
        ...
        for (int lvt_2_1_ = 0; lvt_2_1_ < components.size(); ++lvt_2_1_)
        {
            if (lvt_2_1_ > 0)
            {
                if (lvt_2_1_ == components.size() - 1)
                {
                    lvt_1_1_.appendText(" and ");
                }
                else if (lvt_2_1_ > 0)
                {
                    lvt_1_1_.appendText(", ");
            ...
            lvt_1_1_.appendSibling((IChatComponent)components.get(lvt_2_1_));
        }
    ...
    public static List<String> func_175771_a(String[] p_175771_0_, int p_175771_1_, BlockPos p_175771_2_)
    {
        ...
        else
        {
            ...
            {
                lvt_3_1_ = Integer.toString(p_175771_2_.getX());
            }
            else if (lvt_4_1_ == p_175771_1_ + 1)
            {
                lvt_3_1_ = Integer.toString(p_175771_2_.getY());
            }
            else
            {
                ...
                lvt_3_1_ = Integer.toString(p_175771_2_.getZ());
            }
    ...
    public static List<String> func_181043_b(String[] p_181043_0_, int p_181043_1_, BlockPos p_181043_2_)
    {
        ...
        else
        {
            ...
            {
                lvt_3_1_ = Integer.toString(p_181043_2_.getX());
            }
            else
            {
                ...
                lvt_3_1_ = Integer.toString(p_181043_2_.getZ());
            }
    ...
    public static List<String> getListOfStringsMatchingLastWord(String[] p_175762_0_, Collection<?> p_175762_1_)
    {
        ...
        if (!p_175762_1_.isEmpty())
        {
            ...
            {
                if (doesStringStartWith(lvt_2_1_, lvt_5_1_))
                {
            ...
            if (lvt_3_1_.isEmpty())
            {
                ...
                {
                    if (lvt_5_2_ instanceof ResourceLocation && doesStringStartWith(lvt_2_1_, ((ResourceLocation)lvt_5_2_).getResourcePath()))
                    {
