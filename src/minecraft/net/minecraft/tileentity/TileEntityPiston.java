...
public class TileEntityPiston extends TileEntity implements ITickable
{
    ...
    private void launchWithSlimeBlock(float p_145863_1_, float p_145863_2_)
    {
        ...
        AxisAlignedBB lvt_3_1_ = Blocks.piston_extension.getBoundingBox(this.worldObj, this.pos, this.pistonState, p_145863_1_, this.pistonFacing);

        if (lvt_3_1_ != null)
        {
            List<Entity> lvt_4_1_ = this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)null, lvt_3_1_);

            if (!lvt_4_1_.isEmpty())
            {
                this.field_174933_k.addAll(lvt_4_1_);

                for (Entity lvt_6_1_ : this.field_174933_k)
                {
                    if (this.pistonState.getBlock() == Blocks.slime_block && this.extending)
                    {
                        ...
                        {
                            case X:
                                lvt_6_1_.motionX = (double)this.pistonFacing.getFrontOffsetX();
                                break;

                            case Y:
                                lvt_6_1_.motionY = (double)this.pistonFacing.getFrontOffsetY();
                                break;

                            case Z:
                                lvt_6_1_.motionZ = (double)this.pistonFacing.getFrontOffsetZ();
                        }
                    }
                    else
                    {
                        lvt_6_1_.moveEntity((double)(p_145863_2_ * (float)this.pistonFacing.getFrontOffsetX()), (double)(p_145863_2_ * (float)this.pistonFacing.getFrontOffsetY()), (double)(p_145863_2_ * (float)this.pistonFacing.getFrontOffsetZ()));
                    }
                }
