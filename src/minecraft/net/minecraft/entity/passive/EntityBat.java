...
public class EntityBat extends EntityAmbientCreature
{
    ...
    protected void updateAITasks()
    {
        ...
        if (this.getIsBatHanging())
        {
            if (!this.worldObj.getBlockState(lvt_2_1_).getBlock().isNormalCube())
            {
                this.setIsBatHanging(false);
                this.worldObj.playAuxSFXAtEntity((EntityPlayer)null, 1015, lvt_1_1_, 0);
            }
            else
            {
        ...
        else
        {
            ...
            this.motionX += (Math.signum(lvt_3_1_) * 0.5D - this.motionX) * 0.10000000149011612D;
            this.motionY += (Math.signum(lvt_5_1_) * 0.699999988079071D - this.motionY) * 0.10000000149011612D;
            this.motionZ += (Math.signum(lvt_7_1_) * 0.5D - this.motionZ) * 0.10000000149011612D;
            float lvt_9_1_ = (float)(MathHelper.atan2(this.motionZ, this.motionX) * 180.0D / Math.PI) - 90.0F;
            float lvt_10_1_ = MathHelper.wrapAngleTo180_float(lvt_9_1_ - this.rotationYaw);
            this.moveForward = 0.5F;
            this.rotationYaw += lvt_10_1_;

            if (this.rand.nextInt(100) == 0 && this.worldObj.getBlockState(lvt_2_1_).getBlock().isNormalCube())
            {
                this.setIsBatHanging(true);
            }
        }
    ...
    public boolean getCanSpawnHere()
    {
        ...
        if (lvt_1_1_.getY() >= this.worldObj.getSeaLevel())
        {
            return false;
        }
        else
        {
            ...
            return lvt_2_1_ > this.rand.nextInt(lvt_3_1_) ? false : super.getCanSpawnHere();
        }
    }
