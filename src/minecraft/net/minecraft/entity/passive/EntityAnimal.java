...
public abstract class EntityAnimal extends EntityAgeable implements IAnimals
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.inLove > 0)
        {
            ...
            if (this.inLove % 10 == 0)
            {
                ...
                this.worldObj.spawnParticle(EnumParticleTypes.HEART, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + 0.5D + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, lvt_1_1_, lvt_3_1_, lvt_5_1_, new int[0]);
            }
        }
    }
    ...
    public boolean getCanSpawnHere()
    {
        ...
        return this.worldObj.getBlockState(lvt_4_1_.down()).getBlock() == this.spawnableBlock && this.worldObj.getLight(lvt_4_1_) > 8 && super.getCanSpawnHere();
    }
    ...
    public boolean interact(EntityPlayer player)
    {
        ...
        if (lvt_2_1_ != null)
        {
            if (this.isBreedingItem(lvt_2_1_) && this.getGrowingAge() == 0 && this.inLove <= 0)
            {
                this.consumeItemFromStack(player, lvt_2_1_);
                this.setInLove(player);
                return true;
            }

            if (this.isChild() && this.isBreedingItem(lvt_2_1_))
            {
                this.consumeItemFromStack(player, lvt_2_1_);
    ...
    public void handleStatusUpdate(byte id)
    {
        if (id == 18)
        {
            for (int lvt_2_1_ = 0; lvt_2_1_ < 7; ++lvt_2_1_)
            {
                ...
                this.worldObj.spawnParticle(EnumParticleTypes.HEART, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + 0.5D + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, lvt_3_1_, lvt_5_1_, lvt_7_1_, new int[0]);
            }
        }
        else
        {
