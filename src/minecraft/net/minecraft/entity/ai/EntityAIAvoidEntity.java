...
public class EntityAIAvoidEntity<T extends Entity> extends EntityAIBase
{
    ...
    {
        List<T> lvt_1_1_ = this.theEntity.worldObj.<T>getEntitiesWithinAABB(this.classToAvoid, this.theEntity.getEntityBoundingBox().expand((double)this.avoidDistance, 3.0D, (double)this.avoidDistance), Predicates.and(new Predicate[] {EntitySelectors.NOT_SPECTATING, this.canBeSeenSelector, this.avoidTargetSelector}));
        ...
        else
        {
            ...
            Vec3 lvt_2_1_ = RandomPositionGenerator.findRandomTargetBlockAwayFrom(this.theEntity, 16, 7, new Vec3(this.closestLivingEntity.posX, this.closestLivingEntity.posY, this.closestLivingEntity.posZ));

            if (lvt_2_1_ == null)
            {
                return false;
            }
            else if (this.closestLivingEntity.getDistanceSq(lvt_2_1_.xCoord, lvt_2_1_.yCoord, lvt_2_1_.zCoord) < this.closestLivingEntity.getDistanceSqToEntity(this.theEntity))
            {
                return false;
            }
            else
            {
                ...
                return this.entityPathEntity == null ? false : this.entityPathEntity.isDestinationSame(lvt_2_1_);
            }
        }
    }
