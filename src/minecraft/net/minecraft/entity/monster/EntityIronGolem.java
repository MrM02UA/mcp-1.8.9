...
public class EntityIronGolem extends EntityGolem
{
    ...
    protected void updateAITasks()
    {
        if (--this.homeCheckTimer <= 0)
        {
            ...
            {
                BlockPos lvt_1_1_ = this.villageObj.getCenter();
                this.setHomePosAndDistance(lvt_1_1_, (int)((float)this.villageObj.getVillageRadius() * 0.6F));
            }
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.motionX * this.motionX + this.motionZ * this.motionZ > 2.500000277905201E-7D && this.rand.nextInt(5) == 0)
        {
            ...
            IBlockState lvt_4_1_ = this.worldObj.getBlockState(new BlockPos(lvt_1_1_, lvt_2_1_, lvt_3_1_));
            Block lvt_5_1_ = lvt_4_1_.getBlock();

            if (lvt_5_1_.getMaterial() != Material.air)
            {
                this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, this.getEntityBoundingBox().minY + 0.1D, this.posZ + ((double)this.rand.nextFloat() - 0.5D) * (double)this.width, 4.0D * ((double)this.rand.nextFloat() - 0.5D), 0.5D, ((double)this.rand.nextFloat() - 0.5D) * 4.0D, new int[] {Block.getStateId(lvt_4_1_)});
            }
        }
    }
    ...
    public boolean attackEntityAsMob(Entity entityIn)
    {
        ...
        boolean lvt_2_1_ = entityIn.attackEntityFrom(DamageSource.causeMobDamage(this), (float)(7 + this.rand.nextInt(15)));
    ...
    protected void dropFewItems(boolean wasRecentlyHit, int lootingModifier)
    {
        ...
        int lvt_4_2_ = 3 + this.rand.nextInt(3);
            ...
            this.targetEntitySelector = new Predicate<T>()
            {
                public boolean apply(T p_apply_1_)
                {
                    ...
                    else
                    {
                        ...
                        {
                            double lvt_2_1_ = AINearestAttackableTargetNonCreeper.this.getTargetDistance();
                            ...
                            if ((double)p_apply_1_.getDistanceToEntity(creature) > lvt_2_1_)
                            {
                                return false;
                            }
                        }
