...
public class EntityCreeper extends EntityMob
{
    ...
    public void onUpdate()
    {
        if (this.isEntityAlive())
        {
            ...
            int lvt_1_1_ = this.getCreeperState();

            if (lvt_1_1_ > 0 && this.timeSinceIgnited == 0)
            {
    ...
    protected boolean interact(EntityPlayer player)
    {
        ...
        if (lvt_2_1_ != null && lvt_2_1_.getItem() == Items.flint_and_steel)
        {
    ...
    private void explode()
    {
        if (!this.worldObj.isRemote)
        {
            boolean lvt_1_1_ = this.worldObj.getGameRules().getBoolean("mobGriefing");
            float lvt_2_1_ = this.getPowered() ? 2.0F : 1.0F;
            this.worldObj.createExplosion(this, this.posX, this.posY, this.posZ, (float)this.explosionRadius * lvt_2_1_, lvt_1_1_);
            this.setDead();
        }
    }
