...
public class SlotCrafting extends Slot
{
    ...
    {
        this.onCrafting(stack);
        ItemStack[] lvt_3_1_ = CraftingManager.getInstance().func_180303_b(this.craftMatrix, playerIn.worldObj);

        for (int lvt_4_1_ = 0; lvt_4_1_ < lvt_3_1_.length; ++lvt_4_1_)
        {
            ItemStack lvt_5_1_ = this.craftMatrix.getStackInSlot(lvt_4_1_);
            ItemStack lvt_6_1_ = lvt_3_1_[lvt_4_1_];

            if (lvt_5_1_ != null)
            {
                this.craftMatrix.decrStackSize(lvt_4_1_, 1);
            }

            if (lvt_6_1_ != null)
            {
                if (this.craftMatrix.getStackInSlot(lvt_4_1_) == null)
                {
                    this.craftMatrix.setInventorySlotContents(lvt_4_1_, lvt_6_1_);
                }
                else if (!this.thePlayer.inventory.addItemStackToInventory(lvt_6_1_))
                {
                    this.thePlayer.dropPlayerItemWithRandomChoice(lvt_6_1_, false);
                }
            }
        }
    }
