...
public class NetHandlerLoginClient implements INetHandlerLoginClient
{
    ...
    {
        final SecretKey lvt_2_1_ = CryptManager.createNewSharedKey();
        String lvt_3_1_ = packetIn.getServerId();
        PublicKey lvt_4_1_ = packetIn.getPublicKey();
        String lvt_5_1_ = (new BigInteger(CryptManager.getServerIdHash(lvt_3_1_, lvt_4_1_, lvt_2_1_))).toString(16);

        if (this.mc.getCurrentServerData() != null && this.mc.getCurrentServerData().isOnLAN())
        {
            try
            {
                this.getSessionService().joinServer(this.mc.getSession().getProfile(), this.mc.getSession().getToken(), lvt_5_1_);
            }
        ...
        this.networkManager.sendPacket(new C01PacketEncryptionResponse(lvt_2_1_, lvt_4_1_, packetIn.getVerifyToken()), new GenericFutureListener < Future <? super Void >> ()
        {
            public void operationComplete(Future <? super Void > p_operationComplete_1_) throws Exception
            {
                NetHandlerLoginClient.this.networkManager.enableEncryption(lvt_2_1_);
            }
