...
public class MusicTicker implements ITickable
{
    ...
    public void update()
    {
        MusicTicker.MusicType lvt_1_1_ = this.mc.getAmbientMusicType();

        if (this.currentMusic != null)
        {
            if (!lvt_1_1_.getMusicLocation().equals(this.currentMusic.getSoundLocation()))
            {
                this.mc.getSoundHandler().stopSound(this.currentMusic);
                this.timeUntilNextMusic = MathHelper.getRandomIntegerInRange(this.rand, 0, lvt_1_1_.getMinDelay() / 2);
            }

            if (!this.mc.getSoundHandler().isSoundPlaying(this.currentMusic))
            {
                this.currentMusic = null;
                this.timeUntilNextMusic = Math.min(MathHelper.getRandomIntegerInRange(this.rand, lvt_1_1_.getMinDelay(), lvt_1_1_.getMaxDelay()), this.timeUntilNextMusic);
            }
        }
