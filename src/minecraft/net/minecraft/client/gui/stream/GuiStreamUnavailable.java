...
public class GuiStreamUnavailable extends GuiScreen
{
    ...
    public void initGui()
    {
        if (this.field_152323_r.isEmpty())
        {
            ...
            if (this.field_152327_i != null)
            {
                ...
                {
                    this.field_152323_r.add(lvt_2_1_.getUnformattedTextForChat());
                }
            }
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        int lvt_4_1_ = Math.max((int)((double)this.height * 0.85D / 2.0D - (double)((float)(this.field_152323_r.size() * this.fontRendererObj.FONT_HEIGHT) / 2.0F)), 50);
        this.drawCenteredString(this.fontRendererObj, this.field_152324_f.getFormattedText(), this.width / 2, lvt_4_1_ - this.fontRendererObj.FONT_HEIGHT * 2, 16777215);

        for (String lvt_6_1_ : this.field_152323_r)
        {
            this.drawCenteredString(this.fontRendererObj, lvt_6_1_, this.width / 2, lvt_4_1_, 10526880);
    ...
    public static void func_152321_a(GuiScreen p_152321_0_)
    {
        ...
        IStream lvt_2_1_ = lvt_1_1_.getTwitchStream();

        if (!OpenGlHelper.framebufferSupported)
        {
            List<ChatComponentTranslation> lvt_3_1_ = Lists.newArrayList();
            lvt_3_1_.add(new ChatComponentTranslation("stream.unavailable.no_fbo.version", new Object[] {GL11.glGetString(GL11.GL_VERSION)}));
            ...
            lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.NO_FBO, lvt_3_1_));
        }
        else if (lvt_2_1_ instanceof NullStream)
        {
            ...
            {
                lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.LIBRARY_ARCH_MISMATCH));
            }
            else
            {
                lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.LIBRARY_FAILURE));
            }
        }
        else if (!lvt_2_1_.func_152928_D() && lvt_2_1_.func_152912_E() == ErrorCode.TTV_EC_OS_TOO_OLD)
        {
            ...
            {
                case WINDOWS:
                    lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_WINDOWS));
                    break;

                case OSX:
                    lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_MAC));
                    break;

                default:
                    lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_OTHER));
            }
        }
        else if (!lvt_1_1_.getTwitchDetails().containsKey("twitch_access_token"))
        {
            if (lvt_1_1_.getSession().getSessionType() == Session.Type.LEGACY)
            {
                lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.ACCOUNT_NOT_MIGRATED));
            }
            else
            {
                lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.ACCOUNT_NOT_BOUND));
            }
        }
        else if (!lvt_2_1_.func_152913_F())
        {
            ...
            {
                case INVALID_TOKEN:
                    lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.FAILED_TWITCH_AUTH));
                    break;

                case ERROR:
                default:
                    lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.FAILED_TWITCH_AUTH_ERROR));
            }
        }
        else if (lvt_2_1_.func_152912_E() != null)
        {
            ...
            lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.INITIALIZATION_FAILURE, lvt_3_2_));
        }
        else
        {
            lvt_1_1_.displayGuiScreen(new GuiStreamUnavailable(p_152321_0_, GuiStreamUnavailable.Reason.UNKNOWN));
        }
    }
