...
public class GuiLockIconButton extends GuiButton
{
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY)
    {
        if (this.visible)
        {
            ...
            GuiLockIconButton.Icon lvt_5_1_;

            if (this.field_175231_o)
            {
                if (!this.enabled)
                {
                    lvt_5_1_ = GuiLockIconButton.Icon.LOCKED_DISABLED;
                }
                else if (lvt_4_1_)
                {
                    lvt_5_1_ = GuiLockIconButton.Icon.LOCKED_HOVER;
                }
                else
                {
                    lvt_5_1_ = GuiLockIconButton.Icon.LOCKED;
                }
            }
            else if (!this.enabled)
            {
                lvt_5_1_ = GuiLockIconButton.Icon.UNLOCKED_DISABLED;
            }
            else if (lvt_4_1_)
            {
                lvt_5_1_ = GuiLockIconButton.Icon.UNLOCKED_HOVER;
            }
            else
            {
                lvt_5_1_ = GuiLockIconButton.Icon.UNLOCKED;
            }

            this.drawTexturedModalRect(this.xPosition, this.yPosition, lvt_5_1_.func_178910_a(), lvt_5_1_.func_178912_b(), this.width, this.height);
        }
    }
