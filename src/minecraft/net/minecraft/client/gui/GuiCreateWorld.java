...
public class GuiCreateWorld extends GuiScreen
{
    ...
    private void calcSaveDirName()
    {
        ...
        for (char lvt_4_1_ : ChatAllowedCharacters.allowedCharactersArray)
        {
            this.saveDirName = this.saveDirName.replace(lvt_4_1_, '_');
        }
    ...
    public static String getUncollidingSaveDirName(ISaveFormat saveLoader, String name)
    {
        ...
        for (String lvt_5_1_ : disallowedFilenames)
        {
            if (name.equalsIgnoreCase(lvt_5_1_))
            {
                name = "_" + name + "_";
            }
        }
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        if (button.enabled)
        {
            ...
            else if (button.id == 0)
            {
                ...
                String lvt_4_1_ = this.worldSeedField.getText();
                ...
                WorldSettings.GameType lvt_5_3_ = WorldSettings.GameType.getByName(this.gameMode);
                WorldSettings lvt_6_1_ = new WorldSettings(lvt_2_1_, lvt_5_3_, this.generateStructuresEnabled, this.hardCoreMode, WorldType.worldTypes[this.selectedIndex]);
                lvt_6_1_.setWorldName(this.chunkProviderSettingsJson);

                if (this.bonusChestEnabled && !this.hardCoreMode)
                {
                    lvt_6_1_.enableBonusChest();
                }

                if (this.allowCheats && !this.hardCoreMode)
                {
                    lvt_6_1_.enableCommands();
                }

                this.mc.launchIntegratedServer(this.saveDirName, this.worldNameField.getText().trim(), lvt_6_1_);
            }
            else if (button.id == 3)
            {
    ...
    {
        WorldType lvt_1_1_ = WorldType.worldTypes[this.selectedIndex];
        return lvt_1_1_ != null && lvt_1_1_.getCanBeCreated() ? (lvt_1_1_ == WorldType.DEBUG_WORLD ? isShiftKeyDown() : true) : false;
    }
