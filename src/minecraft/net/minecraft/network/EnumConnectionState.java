...
public enum EnumConnectionState
{
    ...
    {
        BiMap < Integer, Class <? extends Packet >> lvt_3_1_ = (BiMap)this.directionMaps.get(direction);

        if (lvt_3_1_ == null)
        {
            lvt_3_1_ = HashBiMap.create();
            this.directionMaps.put(direction, lvt_3_1_);
        }

        if (lvt_3_1_.containsValue(packetClass))
        {
            String lvt_4_1_ = direction + " packet " + packetClass + " is already known to ID " + lvt_3_1_.inverse().get(packetClass);
            LogManager.getLogger().fatal(lvt_4_1_);
            throw new IllegalArgumentException(lvt_4_1_);
        }
        else
        {
            lvt_3_1_.put(Integer.valueOf(lvt_3_1_.size()), packetClass);
            return this;
        }
    }
    ...
    public Packet getPacket(EnumPacketDirection direction, int packetId) throws InstantiationException, IllegalAccessException {
        Class <? extends Packet > lvt_3_1_ = (Class)((BiMap)this.directionMaps.get(direction)).get(Integer.valueOf(packetId));
    ...
    static {
        for (EnumConnectionState lvt_3_1_ : values())
        {
            ...
            STATES_BY_ID[lvt_4_1_ - field_181136_e] = lvt_3_1_;

            for (EnumPacketDirection lvt_6_1_ : lvt_3_1_.directionMaps.keySet())
            {
                for (Class <? extends Packet > lvt_8_1_ : ((BiMap)lvt_3_1_.directionMaps.get(lvt_6_1_)).values())
                {
                    if (STATES_BY_CLASS.containsKey(lvt_8_1_) && STATES_BY_CLASS.get(lvt_8_1_) != lvt_3_1_)
                    {
                        throw new Error("Packet " + lvt_8_1_ + " is already assigned to protocol " + STATES_BY_CLASS.get(lvt_8_1_) + " - can\'t reassign to " + lvt_3_1_);
                    }
